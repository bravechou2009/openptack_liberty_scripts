...
apt-get -y install neutron-plugin-mlnx neutron-plugin-mlnx-agent

wget -c http://www.mellanox.com/repository/solutions/openstack/liberty/ubuntu/14.04/python-networking-mlnx_7.0.0-1_all.deb
wget -c http://www.mellanox.com/repository/solutions/openstack/liberty/ubuntu/14.04/eswitchd_1.0.0-18_amd64.deb

dpkg -i python-networking-mlnx_7.0.0-1_all.deb
dpkg -i eswitchd_1.0.0-18_amd64.deb
fi
...
filename=/etc/eswitchd/eswitchd.conf
test -f $filename.org || cp $filename $filename.org
rm $filename

cat << EOF >> $filename
[DEFAULT]
log_file=/var/log/eswitchd/eswitchd.log
log_level=DEBUG
[DAEMON]
fabrics=$PHYSICAL_NETWORK:$DEVNAME
EOF
...
cat << EOF > $filename
[eswitch]
physical_interface_mappings = $PHYSICAL_NETWORK:$DEVNAME
...
#ln -sf /usr/bin/neutron-mlnx-agent /usr/bin/neutron-plugin-mlnx-agent
...
###################################################################################################################
###################################################################################################################
apt-get -y remove --purge neutron-server neutron-plugin-ml2 neutron-plugin-openvswitch-agent \
...
apt-get -y  remove --purge --force-yes neutron-plugin-mlnx neutron-plugin-mlnx-agent

apt-get -y  remove --purge neutron-plugin-sriov-agent


apt-get install neutron-server neutron-plugin-ml2 neutron-plugin-openvswitch-agent \
neutron-l3-agent neutron-dhcp-agent neutron-metadata-agent neutron-plugin-openvswitch neutron-common conntrack


apt-get install --force-yes neutron-plugin-mlnx neutron-plugin-mlnx-agent eswitchd python-networking-mlnx 

apt-get install neutron-server neutron-plugin-ml2 neutron-l3-agent \
neutron-dhcp-agent neutron-metadata-agent neutron-plugin-openvswitch neutron-plugin-openvswitch-agent
...
port_id=`neutron port-create $net_id --name sriov_port --binding:vnic-type direct | grep "\ id\ " | awk '{ print $4 }'`
nova boot --flavor m1.small --image ubuntu-mlnx-dhcp --nic port-id=$port_id test-sriov
nova boot --flavor m1.tiny --image cirros-0.3.4-x86_64 --nic port-id=$port_id test-sriov
...
neutron port-delete $(neutron port-list | grep "\ sriov_port\ " | awk '{ print $2 }')
net_id=`neutron net-show sriov_254.x | grep "\ id\ " | awk '{ print $4 }'`
port_id=`neutron port-create $net_id --name sriov_port --vnic-type direct | grep "\ id\ " | awk '{ print $4 }'`
nova boot --flavor m1.tiny --image TestVM --nic port-id=$port_id test-sriov
...
ipmi -I lanplus -H 192.168.111.5 -U root -P lovedcn123 sol activate